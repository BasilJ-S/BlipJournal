...
struct NewQuestionSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Add") { add() }.disabled(label.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || (kind == .scale && !scale.isValid)) } }
    ...
    private func description(_ kind: QuestionKind) -> String { switch kind { case .scale: "Scale — choose a number"; case .singleChoice: "Single choice — choose one"; case .multiChoice: "Multiple choice — choose any"; case .yesNo: "Yes or no"; case .text: "Quick note — short text" } }
    private func add() {
        do { let q = try EditorModel(store: appModel.store, notifications: appModel.notifications).addQuestion(surveyId: surveyId, kind: kind, label: label, isRequired: required, scale: kind == .scale ? scale : nil, allowsCustomOptions: kind.usesOptions && customOptions); for (i, value) in options.split(separator: "\n").map(String.init).enumerated() { _ = try EditorModel(store: appModel.store, notifications: appModel.notifications).addOption(questionId: q.id, label: value) ; _ = i }; try appModel.refresh(); dismiss() } catch { errorMessage = String(describing: error) }
    }
}
