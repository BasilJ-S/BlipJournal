...
struct SamplingSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Notification preview") {
                ...
                if let survey = try? appModel.store.survey(surveyId) { let content = preview.content(surveyName: survey.name); Text("Title: \(content.title)"); Text("Body: \(content.body)") }
            }
        ...
        .toolbar { ToolbarItem(placement: .confirmationAction) { Button("Save") { save() }.disabled(!errors.isEmpty || (config.isEnabled && ((try? appModel.store.survey(surveyId))?.map { $0.activeQuestions.isEmpty } ?? false)) || !preview.isValid) } }
        .onAppear { load() }
    ...
    private func save() { if case .custom = preview { preview = .custom(message: customMessage) }; Task { do { let model = EditorModel(store: appModel.store, notifications: appModel.notifications); try await model.saveSampling(surveyId: surveyId, config); try await model.saveNotificationPreview(surveyId: surveyId, preview); try appModel.refresh(); dismiss() } catch { errorMessage = String(describing: error) } } }
